    ...
    >
      ...
      <div className="relative flex flex-col items-center justify-center pt-6 pb-4 px-4 shrink-0">
        <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-[#E8A317] to-[#D4920B] shadow-[0_2px_10px_rgba(232,163,23,0.3)] flex items-center justify-center shrink-0">
          <span className="font-extrabold text-[#0B1D3A] text-[15px] leading-none tracking-tight">LT</span>
        </div>
        <div className={`overflow-hidden transition-all duration-300 ${isExpanded ? 'opacity-100 max-h-10 mt-2' : 'opacity-0 max-h-0'}`}>
          <span className="font-extrabold text-white text-[18px] whitespace-nowrap tracking-tight">LekkerTime</span>
        </div>
        {isExpanded && (
          <button
      ...
      <nav className="flex-1 px-3 py-4 space-y-1 overflow-y-auto overflow-x-hidden no-scrollbar">
        {navItems.map((item) => (
          ...
          >
            {({ isActive }) => (
              <>
                <item.icon size={22} className={`shrink-0 ${isActive ? 'text-[#0B1D3A]' : ''}`} />
                <span
      ...
      <div className="p-4 border-t border-white/10 shrink-0">
        ...
        >
          ...
          >
            <div className="text-sm font-medium text-white whitespace-nowrap">Michael Scott</div>
            <div className="text-xs text-white/50 whitespace-nowrap">Regional Manager</div>
          </div>
        </div>
      </div>
    </aside>
